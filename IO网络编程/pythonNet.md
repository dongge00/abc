...
参数：file_name  文件名；
     access_mode  打开文件的方式,如果不写默认为‘r’ 
          文件模式                        操作
              r                    以读方式打开.字符串方式显示 文件必须存在
              w                    以写方式打开
                                   文件不存在则创建(以参数中的file_name名字命名,在当前文件夹创建)，存在清空原有内容 
              a                    以追加模式打开,也是写.文件不存在则创建,不清空 
              r+                   以读写模式打开 文件必须存在
              w+                   以读写模式打开文件
                                   不存在则创建，存在清空原有内容
              a+                   以读写模式打开 追加模式
              rb                   以二进制读模式打开,字节串方式显示 同r
              wb                   以二进制写模式打开 同w
              ab                   以二进制追加模式打开 同a
              rb+                  以二进制读写模式打开 同r+
              wb+                  以二进制读写模式打开 同w+
              ab+                  以二进制读写模式打开 同a+
     buffering  1表示有行缓冲，默认则表示使用系统默认提供的缓冲机制。-1 是默认.
返回值：成功返回文件操作对象。
```
[

>>> chr(90)    # 把ASCIIm码转换成字符串
'Z'
>>> ord("a")     # 把字符串转换成ASCIIm码
97
>>> 
encode()  # 把字符串,转换成二进制
...
1. 读取文件 : 凡是从外部获取,进入内存的操作,就是读取操作.

>read([size])
>功能： 来直接读取文件中字符。
>参数： 如果没有给定size参数（默认值为-1）或者size值为负，文件将被读取直至末尾，给定size最多读取给定数目个字符（字节）。比如: size,填写 1M,就是1024个字节的内容.
>返回值： 返回读取到的内容
>
...
5. with操作 : 文件对象,可以通过with语句.但不是所有的对象,都可用with
...
```python
with open('file','r+') as f:   # 打开文件,并 生成 文件对象,as取名 为 f 
    f.read()
```
...
>>IPv4  点分十进制表示 172.40.91.185 每部分取值范围0--255
        [每部分0-255,就是 2的8次方等于256,就是 11111111 8个二进制位,就是一个字节,ipv4 就是4个字节,32位二进制表示.]
>>IPv6  128位 扩大了地址范围,
        [ipv6  2个16进制为一组.一个16进制占4位.]


* 域名 :[ 理解为 身份证号(ip) 与 姓名(域名) .]
>定义： 给网络服务器地址起的名字

>作用： 方便记忆，表达一定的含义

>ping [ip] : 测试和某个主机是否联通
[比如在 终端,输入 ping  www.baidu.com ,可查联通百度主机]

* 端口号（port）
>作用：端口是网络地址的一部分，用于区分主机上不同的网络应用程序。
       [每台主机,一个应用,就配一个端口.不能重复.取值范围1--65535,  应用端的端口,一般叫客户端有端口.服务器上的服务程序,还有一个程序端口. 当用户登录连接服务器,服务器储存登录用户信息(包含用户的端口信息等), 假设,服务器中有一个微信聊天服务程序,就有一个聊天的程序端口,A与B是朋友,他们都下载了微信,A发信息给腾讯服务器,再经过4层网络模型,传给B.]
...
> 三次握手（建立连接）
>>客户端(client)向服务器发送消息报文请求连接
>>服务器(server)收到请求后，回复报文确定可以连接
>>客户端收到回复，发送最终报文连接建立
...
### 套接字介绍 : [开始是基于unix开发的数据传输的技术,现在通用于各种系统,其他各种后端语言,都有socket模块]

1. 套接字 ： 实现网络编程进行数据传输的一种技术手段

2. Python实现套接字编程：import  socket  导入socker模块,里面保护socket函数

3. 应用层套接字分类[还有其他类型,但目前用不到,比如总线套接字]
>流式套接字(SOCK_STREAM): sock_stream,以字节流方式传输数据，实现tcp协议网络传输方案。(面向连接--tcp协议--可靠的--流式套接字)

>数据报套接字(SOCK_DGRAM):sock_dgram,以数据报形式传输数据，实现udp网络传输方案。(无连接--udp协议--不可靠--数据报套接字)


### tcp套接字编程  [是 流式套接字的另外说法]
...
1. 创建套接字 [像打电话,先要有电话,创建实例对象]

```python
sockfd=socket.socket(socket_family=AF_INET,socket_type=SOCK_STREAM,proto=0)
功能：创建套接字
参数： socket_family  网络地址类型 AF_INET表示ipv4[默认的,如果是用ipv6,是 AF_INET6]
    socket_type  套接字类型 SOCK_STREAM(流式)[不写,默认是流式] 或  SOCK_DGRAM(数据报)
	proto  应用层编程通常为0  选择子协议[因为应用层,就2种套接字,且流式和数据报这2种套接字,都没子协议.有的套接字,是有协议,且有子协议,需选择.此处因无则不需要]
返回值： 套接字对象
```
2. 绑定地址[像电话卡,服务器端必须有一个确定的ip地址和端口 ]
 
>本地地址 ： 'localhost' , '127.0.0.1'
[如果你的服务器上的服务端程序,不希望别人访问,就绑定本地地址,自己的程序未上架,仅自己测试.本地主机上有客户端,也有服务器端,自己测试用.]
>网络地址 ： '172.40.91.185'
[如果服务器设置成网络地址,无论你的客户端在自己主机,还是在别人主机上,都可用网络地址访问服务器上的服务端程序.inconfig查看自己的网络地址]
>自动获取地址： '0.0.0.0'
[自动探测网卡ip,如果服务端绑定是4个0,别人主机上的客户端要访问,用你的网络地址访问该服务器.你自己主机上的客户端访问,用网络地址或者本地地址都可]

![](img/address.png)

```python
sockfd.bind(addr) 
[bind后边只一个参数,且是二元元祖,所以写法是(('0.0.0.0',8888)),如果不括起来,就表示2个参数]
功能： 绑定本机网络地址
...
3. 设置监听[像电话卡充值,缴费]
[一个套接字,可以链接多个客户端.但必须一个个连,等待被处理的客户端存入队列]
```python
sockfd.listen(n)
功能 ： 将套接字设置为监听套接字，确定监听队列大小
参数 ： n 监听队列大小 linux在底层已默认设置了监听队列大小,在应用层设置,实际是无效的.但在 windows下,应用层的监听队列设置,是有效的.
```
4. 等待处理客户端连接请求 [像是让电话处于待机状态]

```python
connfd,addr = sockfd.accept()
功能： 阻塞等待处理客户端请求
 [阻塞函数,是io操作下常用的函数,比如sleep(2)睡觉表示等待2秒]
返回值：一个元祖,里2个值,二元元祖.
        connfd  客户端连接套接字
         addr  连接的客户端地址
```
[ 服务端1个Socket,监听来自客户端的链接,如果来一个链接,就在创建一个Connfg进行通讯,fd 即是文件描述符]

5. 消息收发[开始通电话][客户端,一般先收后发]
...
功能：连接服务器
参数：元组  服务器地址[怎么写,依赖于服务器绑定的地址是哪个,依赖于客户端地址是否与服务器在同一个主机.]
```
...
[ 流式套接字,发送信息,实际发送到缓冲区,recv接收一次,下次再接收,可能一次性把多次发送的消息,接收了.(按照recv里的参数,决定一次接收字节大小) ]

>影响：如果每次发送内容是一个独立的含义，需要接收端独立解析此时粘包会有影响。
[把独立含义的两个信息粘包,可能产生歧义.接受者可能以为后信息没发完,在等.实际粘包了,跟前信息一起已发过来了]

>处理方法
>>1. 人为的添加消息边界  [有多种方法,具体没讲太多]
>>2. 控制发送速度
